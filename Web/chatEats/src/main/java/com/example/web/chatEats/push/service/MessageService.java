...
import com.example.web.chatEats.push.bean.card.MessageCard;
import com.example.web.chatEats.push.bean.db.Group;
import com.example.web.chatEats.push.bean.db.Message;
import com.example.web.chatEats.push.bean.db.User;
import com.example.web.chatEats.push.factory.GroupFactory;
import com.example.web.chatEats.push.factory.MessageFactory;
import com.example.web.chatEats.push.factory.PushFactory;
...
public class MessageService extends BaseService {
    ...
    private ResponseModel<MessageCard> pushToUser(User sender, MessageCreateModel model) {
        ...
        if (receiver.getId().equalsIgnoreCase(sender.getId())) {
            ...
            Logger.getLogger (MessageService.class.getName()).log(Level.INFO, "aaaaaa");
    ...
    private ResponseModel<MessageCard> buildAndPushResponse(User sender, Message message) {
        if (message == null) {
            // 存储数据库失败
            Logger.getLogger (MessageService.class.getName()).log(Level.INFO, "bbbbbb");
